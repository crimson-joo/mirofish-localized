...
class OasisProfileGenerator:
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """获取系统提示词"""
        base_prompt = (
            "You are an expert social-media persona generator for public-opinion simulation. "
            "Generate detailed, realistic personas that preserve the known real-world context as much as possible. "
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social-media persona for this entity, preserving known real-world/contextual information as much as possible.
...
2. persona: detailed persona description as one continuous plain-text paragraph, including:
   - basic information (age, profession, education, location)
   - personal background (key experiences, relation to the event, social relationships)
   ...
   - stance/opinions (attitude toward the topic, what may anger or move this person)
   - distinctive traits (catchphrases, special experiences, hobbies)
   - personal memory (important memory connecting this individual to the event and their prior actions/reactions)
3. age: integer
4. gender: English enum only: "male" or "female"
5. mbti: MBTI type, such as INTJ or ENFP
6. country: country/region name in the selected user language
7. profession: profession
8. interested_topics: array of interested topics

Important:
- All values must be strings, numbers, or arrays; do not use newline characters inside string values
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social-media account setting for this organization/group entity, preserving known real-world/contextual information as much as possible.
...
2. persona: detailed account setting as one continuous plain-text paragraph, including:
   - organization basics (official name, nature, background, core functions)
   ...
   - content behavior (content types, posting frequency, active periods)
   - stance/attitude (official stance toward the topic, handling of controversy)
   - special notes (group profile represented, operating habits)
   - organizational memory (important memory connecting this organization to the event and its prior actions/reactions)
3. age: fixed integer 30
4. gender: fixed English enum "other"
5. mbti: MBTI type describing account style, e.g. ISTJ for rigorous/conservative
6. country: country/region name in the selected user language
7. profession: organizational role/function description
8. interested_topics: array of focus areas

Important:
- All values must be strings, numbers, or arrays; do not use null
...
- The account voice must match its identity and positioning"""
