    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <div v-if="phase === 0" class="persona-plan-panel">
            <div class="persona-plan-header">
              <div>
                <span class="section-title">{{ $t('step2.personaPlanTitle') }}</span>
                <p class="description compact">{{ $t('step2.personaPlanDesc') }}</p>
              </div>
            </div>
            <div class="persona-mode-grid">
              <button
                class="persona-mode-card"
                :class="{ active: personaSelectionMode === 'core' }"
                @click="personaSelectionMode = 'core'"
              >
                <strong>{{ $t('step2.personaModeCore') }}</strong>
                <span>{{ $t('step2.personaModeCoreDesc', { count: maxAgentPersonas }) }}</span>
              </button>
              <button
                class="persona-mode-card"
                :class="{ active: personaSelectionMode === 'all' }"
                @click="personaSelectionMode = 'all'"
              >
                <strong>{{ $t('step2.personaModeAll') }}</strong>
                <span>{{ $t('step2.personaModeAllDesc') }}</span>
              </button>
            </div>
            <div v-if="personaSelectionMode === 'core'" class="persona-cap-row">
              <span>{{ $t('step2.maxPersonaCount') }}</span>
              <input type="range" v-model.number="maxAgentPersonas" min="10" max="100" step="5" />
              <strong>{{ maxAgentPersonas }}</strong>
            </div>
            <div class="persona-estimate-row">
              <span>{{ $t('step2.graphMemoryDefaultOn') }}</span>
              <span>{{ personaSelectionMode === 'all' ? $t('step2.executionScaleLarge') : $t('step2.executionScaleBalanced') }}</span>
            </div>
            <button class="action-btn primary wide" @click="startPrepareSimulation">
              {{ $t('step2.startPersonaPreparation') }} ➝
            </button>
          </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            <div class="stat-card">
              <span class="stat-value">{{ extractedEntitiesCount || expectedTotal || profiles.length }}</span>
              <span class="stat-label">{{ $t('step2.extractedEntityCount') }}</span>
            </div>
            <div class="stat-card">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">{{ $t('step2.expectedPersonaTotal') }}</span>
            </div>
            <div class="stat-card">
                ...
                <div class="range-wrapper">
                  ...
                  <div class="range-marks">
                    ...
                    <span 
                      ...
                      :class="{ active: customMaxRounds === DEFAULT_PREVIEW_ROUNDS }"
                      @click="customMaxRounds = DEFAULT_PREVIEW_ROUNDS"
                      :style="{ position: 'absolute', left: `calc(${(DEFAULT_PREVIEW_ROUNDS - 1) / (autoGeneratedRounds - 1) * 100}% - 30px)` }"
                    >{{ $t('step2.recommendedRounds', { rounds: DEFAULT_PREVIEW_ROUNDS }) }}</span>
...
const extractedEntitiesCount = ref(null)
const personaSelectionMode = ref('core')
const maxAgentPersonas = ref(30)
...
const useCustomRounds = ref(true) // 기본은 안전한 프리뷰 실행, 사용자가 전체 라운드로 전환 가능
const customMaxRounds = ref(DEFAULT_PREVIEW_ROUNDS)
...
const autoGeneratedRounds = computed(() => {
  ...
  // 确保最大轮数不小于默认预览值，避免滑动条范围异常
  return Math.max(calculatedRounds, DEFAULT_PREVIEW_ROUNDS)
})
...
const handleStartSimulation = () => {
  ...
  } else if (autoGeneratedRounds.value) {
    // 전체 실행도 명시적으로 라운드 수를 전달해 기록/재실행 시 120라운드가 우발적으로 시작되지 않게 한다.
...
const startPrepareSimulation = async () => {
  ...
  try {
    const res = await prepareSimulation({
      ...
      persona_selection_mode: personaSelectionMode.value,
      max_agent_personas: personaSelectionMode.value === 'all' ? 0 : maxAgentPersonas.value
    })
    ...
    if (res.success && res.data) {
      ...
      // 立即设置预期Persona总数（从prepare接口返回值获取）
      if (res.data.filtered_entities_count) {
        extractedEntitiesCount.value = res.data.filtered_entities_count
      }
      if (res.data.expected_personas_count || res.data.expected_entities_count) {
        expectedTotal.value = res.data.expected_personas_count || res.data.expected_entities_count
        addLog(t('log.zepEntitiesFound', { count: res.data.filtered_entities_count || expectedTotal.value }))
        addLog(t('log.personaPlanApplied', { mode: res.data.persona_selection_mode || personaSelectionMode.value, count: expectedTotal.value }))
...
onMounted(() => {
  if (props.simulationId) {
    ...
    addLog(t('log.personaPlanAwaitingSelection'))
  }
})
...
.persona-mode-card.active {
  border-color: #111;
  box-shadow: 0 0 0 2px rgba(0,0,0,0.08);
}

.persona-mode-card span {
  font-size: 12px;
  color: #666;
  line-height: 1.45;
}
